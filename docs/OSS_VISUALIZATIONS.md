...
问号列名按你的查询来。设置项在可视化「数据 / 显示」里。

### 雷达图 `radar`

适合：多指标对比（营收、转化、留存…）。

- 数据：1 个分类维度 + 1 个或多个数值指标
- 设置：`radar.dimension`（轴），`radar.metrics`（系列）
- 行数建议 ≥ 3
...
- 数据：2 个维度 + 1 个指标
- 设置：`heatmap.x`、`heatmap.y`、`heatmap.value`
...
- 数据：1 个日期列 + 1 个指标
- 设置：`calendar.date`、`calendar.value`
...
- 设置：`bubble.x`、`bubble.y`、`bubble.size`、`bubble.dimension`
...
- 数据：源维度、目标维度，可选边权重
- 设置：`graph.source`、`graph.target`、`graph.value`
...
- 数据：类别 + 开始 + 结束（日期或数值）
- 设置：`gantt.category`、`gantt.start`、`gantt.end`
