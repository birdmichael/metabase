...
import { ROSE_CHART_DEFINITION } from "./definition";

const isSensible = checkNotNull(ROSE_CHART_DEFINITION.isSensible);
...
describe("ROSE_CHART_DEFINITION", () => {
  describe("isSensible", () => {
    it("should return true for a dimension, metric, and at least 2 rows", () => {
      expect(isSensible(createMockDatasetData({ rows, cols: columns }))).toBe(
        true,
      );
    });

    it("should return false with a single row", () => {
      expect(
        isSensible(
  ...
  describe("checkRenderable", () => {
    it("should not throw for valid columns", () => {
      expect(() =>
        ROSE_CHART_DEFINITION.checkRenderable(
          [
            {
              card: createMockCard(),
